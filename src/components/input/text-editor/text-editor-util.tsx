...
export function runCommand(cmd: string, arg?: string | null, event?: any) {
    if (!cmd || !getEditableSelection()) return;
    event?.preventDefault(); // Avoids loosing focus from the editable area
    document.execCommand(cmd, false, arg || undefined); // TODO deprecated
}
...
//     let formatCtl: HTMLElement | undefined;
...
//     if (sel.isCollapsed) {
//         formatCtl = getFormatCtl(range);
//         if (!formatCtl) return;

//         if (!format) formatCtl.className = "";
//         else if (formatCtl.classList.contains(format)) formatCtl.classList.remove(format);
//         else formatCtl.classList.add(format);
//     } else {
//         if (sel.anchorNode != sel.focusNode) return; // Nur Selektionen in einem Element werden unterstützt

//         formatCtl = document.createElement("span");
//         formatCtl.textContent = range.toString();
//         formatCtl.classList.add(format);

//         range.deleteContents();
//         range.insertNode(formatCtl);
//     }
// }

// function getFormatCtl(range: Range): HTMLElement | undefined {
//     let formatCtl = range.commonAncestorContainer as HTMLElement;
//     if (!formatCtl) return;

//     if (!formatCtl.classList) formatCtl = formatCtl.parentElement as HTMLElement;
