...
export const Switch = ({ className, onChange, value, defaultValue, color, size, disabled, ...props }) => {
    return (_jsx(BaseSwitch, { checked: value, defaultChecked: defaultValue, onChange: (checked) => onChange?.({ value: checked }), className: _switch({ className, color, size }), disabled: disabled, ...props, children: _jsx("span", { "aria-hidden": "true", className: "pointer-events-none inline-block size-5 translate-x-0 rounded-full bg-white ring-0 shadow-lg transition duration-200 ease-in-out group-data-checked:translate-x-7" }) }));
};
