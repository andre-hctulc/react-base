...
const chip = tv({
    ...
    variants: {
        ...
        variant: {
            ...
            outlined: "border bg-opacity-0 data-[clickable=true]:hover:bg-opacity-10 data-[clickable=true]:active:bg-opacity-20",
...
export const Chip = React.forwardRef(({ children, color, variant, className, size, hoverEffect, clickable, textSelect, icon, iconPosition, as, thinBorder, thinText, ...props }, ref) => {
    const Comp = as || "span";
    return (_jsxs(Comp, { ref: ref, "data-clickable": clickable, className: chip({ color, variant, size, className, textSelect, thinBorder, thinText }), ...props, children: [icon && iconPosition === "left" && icon, children, icon && iconPosition !== "left" && icon] }));
});
