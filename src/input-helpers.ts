...
import { checkEmail, checkPhoneNumber } from "@client-util/strings";
...
export function convertFormData<T extends Record<string, any> = any>(
    ...
    for (const key in expect) {
        ...
        if (typeof exp === "string") exp = { parse: exp, required: true };
        if (typeof exp !== "object" || !exp || !exp.pattern) continue;
